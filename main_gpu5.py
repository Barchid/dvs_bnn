...
transf = ["crop", "flip_polarity", "event_drop_2"]
learning_rate = 1e-2
epochs = 500
...
def main(dataset):
    ...
    module = BNNModule(learning_rate=learning_rate, n_classes=datamodule.num_classes, epochs=epochs, model="20")
    ...
    trainer = pl.Trainer(
        ...
        logger=pl.loggers.TensorBoardLogger(
            "/sandbox1/sami/new_bnn", name=f"{name}"
        ),
        default_root_dir=f"/sandbox1/sami/new_bnn/{name}",
        precision=16,
    )
