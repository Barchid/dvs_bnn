...
def main(model="18loc"):
    ...
    dataset = NCALTECH101Localization(
        ...
        transform=DVSTransform(
            ...
            transforms_list=["reverse", "flip_polarity", "event_drop_2"],
        )
    )
    ...
    _, val_set = random_split(dataset, [train_len, val_len])
    ...
    trainer = pl.Trainer(
        ...
        logger=pl.loggers.TensorBoardLogger(
            "/datas/sandbox/sami/new_bnn", name=f"{name}"
        ),
        default_root_dir=f"/datas/sandbox/sami/new_bnn/{name}",
        # precision=16,
    )
